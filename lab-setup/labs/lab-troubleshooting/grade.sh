...
if [ "$can_list" = "yes" ] && [ "$can_del" = "no" ]; then
  pass 8 "A1 RBAC — deploy-bot can list but not delete pods" $d
else
  r=""
  [ "$can_list" = "yes" ] || r+="cannot 'list' pods; "
  [ "$can_del" = "no" ]   || r+="can 'delete' pods (too permissive); "
  fail 8 "A1 RBAC — deploy-bot must be able to 'list' but not 'delete' pods" $d "${r%; }"
fi
...
if [ "${avail:-0}" -ge 1 ]; then
  pass 8 "A3 node — w1 fixed (cordon + taint), billing available" $d
else
  u1=$(jp get node w1 -o jsonpath='{.spec.unschedulable}')
  t1=$(jp get node w1 -o jsonpath='{.spec.taints[?(@.key=="maintenance")].effect}')
  fail 8 "A3 node — bring w1 back in service so 'billing' starts" $d "billing available=${avail:-0} ; w1 unschedulable=${u1:-false}, taint maintenance=${t1:-∅}"
fi
...
if [ "$phase" = "Running" ] && [ "$ready" = "true" ]; then
  pass 6 "W2 CrashLoop — crasher runs (container Ready)" $d
else
  ...
  fail 6 "W2 CrashLoop — 'crasher' must run stably (container Ready)" $d "phase=${phase:-absent}, ready=${ready:-?}, restartCount=${rc:-?}${reason:+, state=$reason}"
fi
...
if [ "$eps" -ge 1 ]; then
  pass 7 "N1 endpoints — api-svc has ${eps} endpoint(s)" $d
else fail 7 "N1 endpoints — fix the api-svc selector" $d "api-svc has ${eps} endpoint (expected ≥ 1)"; fi

# N2 — targetPort fixed (7, live test)
d=NET
if jp -n ts-net exec shop-client -- wget -T 4 -qO- http://shop-svc >/dev/null 2>&1; then
  pass 7 "N2 targetPort — shop-client reaches shop-svc" $d
else fail 7 "N2 targetPort — fix shop-svc targetPort (traffic OK)" $d "wget shop-client → shop-svc fails (targetPort ≠ container port?)"; fi

# N3 — NetworkPolicy fixed (7, live test)
d=NET
if jp -n ts-netpol exec client -- wget -T 4 -qO- http://backend >/dev/null 2>&1; then
  pass 7 "N3 NetworkPolicy — client reaches backend" $d
else fail 7 "N3 NetworkPolicy — allow client → backend (traffic OK)" $d "wget client → backend blocked (default-deny not fixed?)"; fi
...
if [ "$ph" = "Bound" ]; then
  pass 7 "S1 PVC — 'data' is Bound" $d
else
  sc=$(jp -n ts-storage get pvc data -o jsonpath='{.spec.storageClassName}')
  fail 7 "S1 PVC — 'data' must bind to a PV (Bound)" $d "phase=${ph:-absent}, storageClassName=${sc:-∅} (no PV in this class?)"
fi
...
if [ "$phase" = "Running" ]; then
  pass 7 "S2 missing PVC — 'app' is Running (PVC created)" $d
else
  ...
  fail 7 "S2 missing PVC — create the PVC referenced by 'app'" $d "phase=${phase:-absent}, PVC '${claim:-app-pvc}'=${exists:-absent}"
fi
