...
# ══════════════════════════════════════════════════════════════════════════════
dom WL 16 "📦 Deployments & Rollouts"

# T1 — Deployment 'frontend' scaled to 3 (8)
d=WL
rr=$(jp -n w-deploy get deploy frontend -o jsonpath='{.status.readyReplicas}')
if [ "${rr:-0}" = "3" ]; then
  pass 8 "T1 frontend — Deployment scaled to 3 ready replicas" $d
else
  fail 8 "T1 frontend — create Deployment 'frontend' (nginx:1.29-alpine) and scale to 3" $d "readyReplicas=${rr:-0} (expected 3)"
fi

# T2 — RollingUpdate strategy maxSurge=2 maxUnavailable=0 (8)
d=WL
...
if [ "$st" = "RollingUpdate" ] && [ "$ms" = "2" ] && [ "$mu" = "0" ]; then
  pass 8 "T2 rollout-app — RollingUpdate maxSurge=2 / maxUnavailable=0" $d
else
  fail 8 "T2 rollout-app — set a RollingUpdate strategy (maxSurge=2, maxUnavailable=0)" $d "type=${st:-∅}, maxSurge=${ms:-∅}, maxUnavailable=${mu:-∅}"
fi

# ══════════════════════════════════════════════════════════════════════════════
dom BATCH 24 "⏱️  DaemonSets, Jobs & CronJobs"

# T3 — DaemonSet ready on all 3 nodes (8)
d=BATCH
nr=$(jp -n w-ds get daemonset node-agent -o jsonpath='{.status.numberReady}')
if [ "${nr:-0}" = "3" ]; then
  pass 8 "T3 node-agent — DaemonSet ready on all 3 nodes (tolerates every taint)" $d
else
  fail 8 "T3 node-agent — DaemonSet must run on ALL nodes incl. cp1 and a tainted w1" $d "numberReady=${nr:-0} (expected 3 — check tolerations)"
fi

# T4 — Job completed 3 times (8)
d=BATCH
sc=$(jp -n w-batch get job pi -o jsonpath='{.status.succeeded}')
if [ "${sc:-0}" = "3" ]; then
  pass 8 "T4 pi — Job reached 3 successful completions" $d
else
  fail 8 "T4 pi — create a Job 'pi' with completions=3" $d "succeeded=${sc:-0} (expected 3)"
fi

# T5 — CronJob schedule every minute (8, graded by spec)
d=BATCH
sch=$(jp -n w-batch get cronjob report -o jsonpath='{.spec.schedule}')
if [ "$sch" = "*/1 * * * *" ]; then
  pass 8 "T5 report — CronJob scheduled every minute" $d
else
  fail 8 "T5 report — create a CronJob 'report' with schedule '*/1 * * * *'" $d "schedule=${sch:-∅}"
fi

# ══════════════════════════════════════════════════════════════════════════════
dom RES 17 "📈 Autoscaling & QoS"

# T6 — HPA min1 max5 cpu50% (9)
d=RES
...
if [ "$mn" = "1" ] && [ "$mx" = "5" ] && [ "$cpu" = "50" ]; then
  pass 9 "T6 hpa-target — HPA min=1 max=5 at 50% CPU" $d
else
  fail 9 "T6 hpa-target — autoscale 'hpa-target' (min=1, max=5, cpu=50%)" $d "min=${mn:-∅}, max=${mx:-∅}, cpu=${cpu:-∅}%"
fi

# T7 — Guaranteed QoS pod (8)
d=RES
qos=$(jp -n w-res get pod guaranteed -o jsonpath='{.status.qosClass}')
if [ "$qos" = "Guaranteed" ]; then
  pass 8 "T7 guaranteed — pod scheduled with QoS class Guaranteed" $d
else
  fail 8 "T7 guaranteed — create a pod whose QoS class is Guaranteed (requests==limits)" $d "qosClass=${qos:-absent}"
fi

# ══════════════════════════════════════════════════════════════════════════════
dom SCHED 43 "🎯 Scheduling & Placement"

# T8 — nodeSelector onto w2 (disktype=ssd) (8)
d=SCHED
...
if [ "$lbl" = "ssd" ] && [ "$n8" = "w2" ] && [ "$p8" = "Running" ]; then
  pass 8 "T8 ssd-pod — Running on w2 via nodeSelector disktype=ssd" $d
else
  fail 8 "T8 ssd-pod — label w2 disktype=ssd and pin 'ssd-pod' there with a nodeSelector" $d "w2 disktype=${lbl:-∅}, pod node=${n8:-none}, phase=${p8:-absent}"
fi

# T9 — nodeAffinity onto w2 (8)
d=SCHED
...
if [ -n "$aff" ] && [ "$n9" = "w2" ] && [ "$p9" = "Running" ]; then
  pass 8 "T9 affinity-pod — Running on w2 via required nodeAffinity" $d
else
  fail 8 "T9 affinity-pod — schedule 'affinity-pod' onto w2 using nodeAffinity (hostname)" $d "affinity set=$([ -n "$aff" ] && echo yes || echo no), node=${n9:-none}, phase=${p9:-absent}"
fi

# T10 — taint w1 + tolerating pod on w1 (9)
d=SCHED
...
if printf '%s' "$taints" | grep -q 'dedicated=batch:NoSchedule' && [ "$n10" = "w1" ] && [ "$p10" = "Running" ]; then
  pass 9 "T10 batch-pod — tolerates w1's taint and runs there" $d
else
  fail 9 "T10 batch-pod — taint w1 (dedicated=batch:NoSchedule) and run a tolerating pod on w1" $d "w1 taints=[${taints}], pod node=${n10:-none}, phase=${p10:-absent}"
fi

# T11 — topologySpreadConstraints spec (9, graded by spec)
d=SCHED
...
if [ "$sms" = "1" ] && [ "$stk" = "kubernetes.io/hostname" ] && [ "$swu" = "ScheduleAnyway" ]; then
  pass 9 "T11 spread-app — topology spread maxSkew=1 over hostname (ScheduleAnyway)" $d
else
  fail 9 "T11 spread-app — add a topologySpreadConstraint (maxSkew=1, hostname, ScheduleAnyway)" $d "maxSkew=${sms:-∅}, topologyKey=${stk:-∅}, whenUnsatisfiable=${swu:-∅}"
fi

# T12 — PriorityClass + critical pod (9)
d=SCHED
...
if [ "$pcv" = "1000000" ] && [ "$pcn" = "high-priority" ] && [ "$p12" = "Running" ]; then
  pass 9 "T12 critical — pod running with PriorityClass high-priority (1000000)" $d
else
  fail 9 "T12 critical — create PriorityClass 'high-priority' (1000000) and use it on pod 'critical'" $d "class value=${pcv:-absent}, pod priorityClassName=${pcn:-∅}, phase=${p12:-absent}"
fi
