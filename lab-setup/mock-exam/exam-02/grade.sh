#!/usr/bin/env bash
# grade.sh — automatic grading for CKA mock exam #2 (advanced level).
# Run ON cp1:  vagrant ssh cp1 -c "bash /vagrant/mock-exam/exam-02/grade.sh"
#
# Makes NO change: read-only. Prints PASS/FAIL per task
...
if jp -n platform get sa ci-bot >/dev/null \
  ...
  pass 7 "T1 RBAC — ci-bot manages Deployments everywhere, without touching nodes" $d
else
  r=""
  jp -n platform get sa ci-bot >/dev/null            || r+="SA platform/ci-bot missing; "
  jp get clusterrole deploy-admin >/dev/null         || r+="ClusterRole deploy-admin missing; "
  jp get clusterrolebinding ci-bot-deploy >/dev/null || r+="ClusterRoleBinding ci-bot-deploy missing; "
  [ "$can_create" = "yes" ] || r+="ci-bot cannot create a Deployment; "
  [ "$can_delnode" = "no" ] || r+="ci-bot can delete nodes (rights too broad); "
  fail 7 "T1 RBAC — SA + ClusterRole + ClusterRoleBinding (deployments cluster-wide)" $d "${r%; }"
fi

# T2 — upgrade control plane cp1 to 1.35 (8) — FINAL TASK (irreversible)
d=ARCH
cpver=$(jp get node cp1 -o jsonpath='{.status.nodeInfo.kubeletVersion}')
if printf '%s' "$cpver" | grep -q '^v1\.35\.'; then
  pass 8 "T2 upgrade — cp1 moved to $cpver (control plane on 1.35)" $d
else fail 8 "T2 upgrade — move control plane cp1 from 1.34 to 1.35 (kubeadm upgrade)" $d "cp1 kubelet=${cpver:-?} (expected v1.35.x)"; fi

# T3 — static pod on w1 with label (5)
d=ARCH
...
if [ "$phase" = "Running" ] && [ "$node" = "w1" ] && [ "$owner" = "Node" ] && [ "$role" = "cache" ]; then
  pass 5 "T3 static pod — static-web-w1 Running on w1 (label role=cache)" $d
else fail 5 "T3 static pod — static-web (label role=cache) via w1's static manifests" $d "phase=${phase:-absent}, node=${node:-?}, owner=${owner:-?}, label role=${role:-∅} (expected Running/w1/Node/cache)"; fi

# T4 — manual scheduling on w2 (5)
d=ARCH
...
if [ "$node" = "w2" ] && [ "$phase" = "Running" ]; then
  pass 5 "T4 manual scheduling — pinned placed on w2" $d
else fail 5 "T4 manual scheduling — pod pinned placed on w2 without the scheduler (nodeName)" $d "node=${node:-?}, phase=${phase:-absent} (expected w2/Running)"; fi

# ══════════════════════════════════════════════════════════════════════════════
dom WORK 15 "📦 Workloads & Scheduling"

# T5 — deployment api + strategy (5)
d=WORK
...
else fail 5 "T5 Deployment api — 3 replicas nginx:1.29-alpine, RollingUpdate maxUnavailable=0" $d "readyReplicas=${rr:-0}, image=${img:-∅}, maxUnavailable=${mu:-∅} (expected 3/nginx:1.29-alpine/0)"; fi

# T6 — secret -> env (5)
d=WORK
skey=$(jp -n apps get secret app-secret -o jsonpath='{.data.TOKEN}')
phase=$(jp -n apps get pod secret-pod -o jsonpath='{.status.phase}')
# We accept the 3 valid forms: env.valueFrom.secretKeyRef, envFrom.secretRef,
# or the runtime proof (TOKEN variable actually present in the container's environment).
...
if [ -n "$skey" ] && [ "$phase" = "Running" ] \
  ...
  pass 5 "T6 Secret→env — TOKEN injected from app-secret" $d
else fail 5 "T6 Secret→env — Secret app-secret(TOKEN) + secret-pod with env from the Secret" $d "secret TOKEN=$([ -n "$skey" ] && echo present || echo absent), pod phase=${phase:-absent}, env from secret=${ref:-${reffrom:-∅}}, TOKEN runtime=$([ -n "$refval" ] && echo present || echo ∅)"; fi
...
if [ "$taint" = "cka" ] && [ "$node" = "w1" ] && [ "$phase" = "Running" ] && [ "$tol" = "dedicated" ]; then
  pass 5 "T7 taint/toleration — w1 tainted, tolerant scheduled on it" $d
else fail 5 "T7 taint/toleration — taint dedicated=cka:NoSchedule on w1 + tolerant pod on it" $d "w1 taint dedicated=${taint:-∅}, pod node=${node:-?}, phase=${phase:-absent}, toleration dedicated=$([ -n "$tol" ] && echo yes || echo no)"; fi

# ══════════════════════════════════════════════════════════════════════════════
dom NET 20 "🌐 Services & Networking"

# T8 — Ingress api-ing (5) — no controller in the lab: we grade the definition
d=NET
...
else fail 5 "T8 Ingress — api-ing (host api.cka.local, path /, backend api-np:80)" $d "host=${ihost:-∅}, backend=${isvc:-∅}:${iport:-∅}, path=${ipath:-∅} (expected api.cka.local/api-np/80/'/')"; fi
...
else fail 5 "T9 NodePort — api-np NodePort 30090 with endpoints" $d "type=${typ:-absent}, nodePort=${np:-∅}, endpoints=${eps} (expected NodePort/30090/≥1)"; fi
...
if printf '%s' "$dd" | grep -qw Ingress && [ "$allow_pod" = "db" ] && [ "$allow_from" = "web" ] && [ "$allow_port" = "80" ]; then
  ...
  r=""
  printf '%s' "$dd" | grep -qw Ingress  || r+="default-deny-ingress missing/incorrect; "
...
# enforcement: web OK, scanner blocked
web=1; scan=1
...
if [ "$web" = "1" ] && [ "$scan" = "1" ]; then
  pass 4 "T10b NetworkPolicy — web reaches db, scanner blocked" $d
else fail 4 "T10b NetworkPolicy — enforcement (web OK / scanner blocked)" $d "web→db=$([ "$web" = 1 ] && echo OK || echo blocked), scanner→db=$([ "$scan" = 1 ] && echo blocked || echo passes)"; fi
...
if [ "$pvcphase" = "Bound" ] && [ "$boundto" = "pv-fast" ] && [ "$reclaim" = "Retain" ]; then
  pass 6 "T11 PV/PVC — data Bound to pv-fast (Retain)" $d
else fail 6 "T11 PV/PVC — pv-fast (Retain) bound to PVC data" $d "pvc phase=${pvcphase:-absent}, bound to=${boundto:-∅}, reclaimPolicy=${reclaim:-∅} (expected Bound/pv-fast/Retain)"; fi

# T12 — pod mounted via subPath (4)
d=STO
...
if [ "$phase" = "Running" ] && [ "$claim" = "data" ] && [ -n "$mp" ] && [ "$sub" = "html" ]; then
  pass 4 "T12 Pod subPath — app mounts data (subPath html) at the right path" $d
else fail 4 "T12 Pod subPath — app mounts PVC data at /usr/share/nginx/html (subPath html)" $d "phase=${phase:-absent}, claim=${claim:-∅}, mount=$([ -n "$mp" ] && echo yes || echo no), subPath=${sub:-∅} (expected Running/data/yes/html)"; fi

# ══════════════════════════════════════════════════════════════════════════════
dom TS 30 "🔧 Troubleshooting"

# T13 — readinessProbe broken (6)
d=TS
avail=$(jp -n trouble get deploy frontend -o jsonpath='{.status.availableReplicas}')
if [ "${avail:-0}" -ge 1 ]; then
  pass 6 "T13 readinessProbe — frontend available" $d
else fail 6 "T13 readinessProbe — fix the frontend probe (pods Ready)" $d "availableReplicas=${avail:-0} (expected ≥1)"; fi

# T14 — DNS resolution (8)
d=TS
...
if [ "$phase" = "Running" ] && timeout 20 kubectl -n trouble exec dns-check -- nslookup kubernetes.default.svc.cluster.local >/dev/null 2>&1; then
  pass 8 "T14 DNS — dns-check resolves cluster names" $d
else fail 8 "T14 DNS — fix the dns-check DNS config (cluster service resolution)" $d "phase=${phase:-absent}, resolution kubernetes.default.svc.cluster.local=$(timeout 20 kubectl -n trouble exec dns-check -- nslookup kubernetes.default.svc.cluster.local >/dev/null 2>&1 && echo OK || echo failed)"; fi
...
if [ "$phase" = "Running" ] && printf '%s' "$img" | grep -q 'nginx:1.29-alpine'; then
  pass 8 "T15 Pending — stuck is Running" $d
else fail 8 "T15 Pending — stuck must run (impossible placement constraint → recreate)" $d "phase=${phase:-absent}, image=${img:-∅}"; fi

# T16 — missing secret created + pods up (8)
d=TS
...
if [ -n "$key" ] && [ "${avail:-0}" -ge 1 ]; then
  pass 8 "T16 Missing Secret — billing-secret created, billing Running" $d
else fail 8 "T16 Missing Secret — create billing-secret(API_KEY) + pods Running" $d "secret API_KEY=$([ -n "$key" ] && echo present || echo absent), availableReplicas=${avail:-0}"; fi

# ══════════════════════════════════════════════════════════════════════════════
echo
echo "────────────────────────────────────────────────────────"
printf "\033[1mSubtotal per domain:\033[0m\n"
